	...
	$('.og_next').click(function(){
		...
		if($('.mainlist li').length>5){//多于4张图片
			...
			if(ml<=0 && ml>w*-1){//默认图片显示时
				$('.swaplist').css({left: '1100px'});//交换图片放在显示区域右侧
				$('.mainlist').animate({left: ml - 1100 + 'px'},'slow');//默认图片滚动				
				if(ml==(w-1000)*-1){//默认图片最后一屏时
					$('.swaplist').animate({left: '0px'},'slow');//交换图片滚动
				}
			}else{//交换图片显示时
				$('.mainlist').css({left: '1100px'})//默认图片放在显示区域右
				$('.swaplist').animate({left: sl - 1100 + 'px'},'slow');//交换图片滚动
